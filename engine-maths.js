...
function showMathsExercise() {
    ...
    const typeConfig = MATHS_TYPES[state.mathsDifficulty] || {};

    // Delegate to custom showExercise if defined (e.g. decomposition)
    if (ex.decompose && typeConfig.showExercise) {
        typeConfig.showExercise(ex);
        return;
    }
    ...
    // Delegate to custom renderHands if defined (e.g. doigts)
    const handsDiv = document.getElementById('maths-hands');
    if (ex.showHands && typeConfig.renderHands) {
        typeConfig.renderHands(ex, handsDiv);
    } else {
